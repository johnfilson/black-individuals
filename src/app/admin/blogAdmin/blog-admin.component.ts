...
export class BlogAdminComponent implements OnInit {
    ...
    getPosts(){
        ...
        dbRef.once('value')
         //once is a promise base method
            .then((snapshot)=> {
                 //data is parse a JSSOn which angular can't directly parse
                let tmp: string[] = snapshot.val();
                this.blogPosts = Object.keys(tmp).map(key => tmp[key])
                //keys abstracts keys from data and the map method to extract the data as a array
            });
    }
